...
function Tree({ children }: { children: ReactNode }): ReactElement {
  return (
    ...
    >
      <div className="nx-inline-block nx-rounded-lg nx-border nx-px-4 nx-py-2 dark:nx-border-neutral-800">{children}</div>
    </div>
  );
}
...
const Folder = memo<FolderProps>(({ label, name, open, children, defaultOpen = false, onToggle }) => {
  ...
  return (
    <li className="nx-flex nx-list-none nx-flex-col">
      <button onClick={toggle} title={name} className="nx-inline-flex nx-cursor-pointer nx-items-center nx-py-1 hover:nx-opacity-60">
        <Ident />
...
const File = memo<FileProps>(({ label, name, active }) => (
  <li className={cn('nx-flex nx-list-none', active && 'nx-text-primary-600 contrast-more:nx-underline')}>
